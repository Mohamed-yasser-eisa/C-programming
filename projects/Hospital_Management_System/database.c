...
uint8 database_save_doctor(uint64 ID, DOCTOR *dr)
{
    ...
    old_doctor = database_check_dr_id(ID, &temp_dr);
    if(1 == old_doctor)
    {
        //doctor is already exist.
        database_edit_doctor(ID, dr);
    }
    else
    {
...
uint8 database_save_patient(uint64 ID, PATIENT *pa)
{
    ...
    old_patient = database_check_pa_n_id(ID, &temp);
    if(1 == old_patient)
    {
        //patient is already exist.
        database_edit_patient(ID, pa);
    }
...
{
    FILE* tfile; //temp file
    FILE* dfile; //doctor file
    DOCTOR td; //temp doctor

    tfile = fopen(file_temp, "w");
    dfile = fopen(file_doctor, "r");
    while( (getc(dfile) != EOF) )//loop until end of file is reached or found doctor.
        {
            fseek(dfile, -1L, SEEK_CUR);
            fscanf(dfile, dr_r_format, &td.nam, &td.gen, &td.spe, &td.n_id, &td.h_id, &td.pho, &td.sal, &td.cons_fees);
            fseek(dfile, 3L, SEEK_CUR); //stop cursor in front of '{'
            /*check if this is the wanted doctor's data or not*/
            if( ID == (td.h_id) )
            {
                //don't copy this doctor;
                fprintf(tfile, dr_w_format, d->nam, d->gen, d->spe, d->n_id, d->h_id, d->pho, d->sal, d->cons_fees);
                continue;
            }//end of if;
            fprintf(tfile, dr_w_format, td.nam, td.gen, td.spe, td.n_id, td.h_id, td.pho, td.sal, td.cons_fees);
        }//end of while();
        fclose(tfile);
        fclose(dfile);
        /*now copy data from temp.pdf to doctors.pdf*/
        tfile = fopen(file_temp, "r");
        dfile = fopen(file_doctor, "w");
        while ( getc(tfile) != EOF)
        {
            fseek(tfile, -1, SEEK_CUR);
            fputc(getc(tfile), dfile);
        }
        fclose(tfile);
        fclose(dfile);
}
